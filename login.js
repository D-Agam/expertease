...
app.post("/checkout", (req, res) => {
  ...
  let user_name = req.body.name; // assuming name is passed from the form
  let user_contact = req.body.phone; // assuming phone is passed from the form
  console.log(services);
  connection.query(
    "INSERT INTO users_hist(name, phonenumber, amount, location, date, services) VALUES (?, ?, ?, ?, ?, ?)",
    [user_name, user_contact, amount, location, date, services],
    function (err, results) {
      if (err) {
        console.error(err);
